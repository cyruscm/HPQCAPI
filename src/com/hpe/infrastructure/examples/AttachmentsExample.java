 ...
 * show how to read/write/update/delete resources, usually entities, this
 * example shows all of those operations for files.
 */
public class AttachmentsExample {

	public static void main(String[] args) throws Exception {
		new AttachmentsExample().attachmentsExample("http://" + Constants.HOST + ":" + Constants.PORT + "/qcbin",
				Constants.DOMAIN, Constants.PROJECT, Constants.USERNAME, Constants.PASSWORD);
	}

	public void attachmentsExample(final String serverUrl, final String domain, final String project, String username,
			String password) throws Exception {

		RestConnector con = RestConnector.getInstance().init(new HashMap<String, String>(), serverUrl, domain, project);

		/*
		 * We use the login example to handle our login for this example. You can view
		 * that code to learn more on authentication.
		 * 
		 * We use the write example to create an entity for us to attach files to. To
		 * learn more on creating entities view that code.
		 * 
		 * We use the update example to lock the entity to which we want to attach
		 * files. This is an "updating" operation. View that code to learn more on
		 * updating.
		 */
		...
		final String createdEntityUrl = writeExample.createEntity(requirementsUrl, Constants.entityToPostXml);
		...
		String newMultiPartAttachmentUrl = example.attachWithMultipart(createdEntityUrl,
				multipartFileContent.getBytes(), "text/plain", multipartFileName, multipartFileDescription);

		String newOctetStreamAttachmentUrl = example.attachWithOctetStream(createdEntityUrl,
				octetstreamFileContent.getBytes(), octetStreamFileName);
		...
		Assert.assertTrue("multipart attachment description missing",
				readAttachments.contains(Constants.generateFieldXml("description", multipartFileDescription)));

		Assert.assertTrue("attachment count incorrect or missing",
		...
		Assert.assertEquals("uploaded octet stream file content differs from read file content", readAttachmentsString,
		...
		Assert.assertEquals("uploaded multipart stream file content differs from read file content",
				readAttachmentsString, multipartFileContent);

		String readAttachmentDetails = example.readAttachmentDetails(newMultiPartAttachmentUrl);
		Assert.assertTrue("multipart attachment description missing",
				readAttachmentDetails.contains(Constants.generateFieldXml("description", multipartFileDescription)));
		...
		example.updateAttachmentData(createdEntityUrl, updatedOctetStreamFileData.getBytes(), octetStreamFileName);
		...
		String attachmentMetadataUpdateResponseXml = example.updateAttachmentDescription(createdEntityUrl,
				updatedOctetstreamFileDescription, octetStreamFileName);

		Assert.assertTrue("updated octet stream description not changed",
	 ...
	 * @param attachmentFileName
	 *            the attachment file name on the server-side whose description we'd
	 *            like to update
	 * @return
	 */
	private String updateAttachmentDescription(String entityUrl, String description, String attachmentFileName)
			throws Exception {
		...
		requestHeaders.put("Accept", "application/xml");
		Response putResponse = con.httpPut(entityUrl + "/attachments/" + attachmentFileName,
				("<Entity Type=\"attachment\"><Fields><Field Name=\"description\"><Value>" + description
	...
	private String readAttachmentDetails(String attachmentUrl) throws Exception {
		...
		/*
		 * A get operation that specifies via accept header that we must have an
		 * application/xml reply. An alt query parameter could also have been used.
		 */
		requestHeaders.put("Accept", "application/xml");
	...
	private byte[] readAttachmentData(String attachmentUrl) throws Exception {
		...
		/*
		 * A get operation that specifies via accept header that we must have an
		 * application/octet-stream reply. An alt query parameter could also have been
		 * used.
		 */
	...
	private String attachWithMultipart(String entityUrl, byte[] fileData, String contentType, String filename,
		 ...
		 * 
		 * //template for file mime part: --<boundary>\r\n Content-Disposition:
		 ...
		 * <value>\r\n <boundary>--
		 * 
		 * //end of parts: --<boundary>--
		 * 
		 * we need 3 parts: filename(template for parameter), description(template for
		 * parameter), and file data(template for file).
		 * 
		 */
