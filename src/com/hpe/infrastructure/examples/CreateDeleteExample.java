...
public class CreateDeleteExample {
	...
	public void createDeleteExample(final String serverUrl, final String domain, final String project, String username,
		...
		/*
		 * Now do the same only this time using an object, and not string xml. (Though
		 * we do build the object from the xml, we could have instantiated it
		 * differently, theoretically.) The reason we build it with post and not
		 * createEntity, is that when posting, the returned value is an xml
		 * representation
		 */
	...
	public String createEntity(String collectionUrl, String postedEntityXml) throws Exception {
		...
		/*
		 * Note that we also get the xml of the newly created entity. at the same time
		 * we get the url where it was created in a location response header.
		 */
