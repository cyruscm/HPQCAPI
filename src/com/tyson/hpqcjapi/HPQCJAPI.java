...
public class HPQCJAPI {
	public static void main(String[] args) {
		...
		Option opt_host = Option.builder("ho").required(false)
				.desc("The host domain to connect to (defaults " + Constants.HOST + ")").longOpt("host").hasArg()
				.build();
		Option opt_port = Option.builder("po").required(false)
				.desc("The port to connect to (defaults " + Constants.PORT + ")").longOpt("port").hasArg().build();
		Option opt_domain = Option.builder("d").required(false)
				.desc("The domain in ALM to connect to. (defaults to " + Constants.DOMAIN + ")").longOpt("domain")
				.hasArg().build();
		Option opt_project = Option.builder("pr").required(false)
				.desc("The project name to connect to (defaults to " + Constants.PROJECT + ")").longOpt("project")
				.hasArg().build();
		Option opt_username = Option.builder("u").required(false)
				.desc("The username to authenticate with. (defaults to " + Constants.USERNAME).longOpt("username")
				.hasArg().build();
		Option opt_password = Option.builder("p").required(false)
				.desc("The password to authenticate with. (defaults to password for " + Constants.USERNAME + ")")
				.longOpt("password").hasArg().build();
